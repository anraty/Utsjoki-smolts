...
s_dat_jags <- function(dat, years, days){
  ...
  data = list(
    Smolts = dat_f %>% select(matches("smolts\\.|smolts$")),
    Schools = dat_f %>% select(matches("schools\\.|schools$")),
    Flow = dat_f %>% select(matches("flow\\.|flow$")),
    Temp = dat_f %>% select(matches("meanTemp\\.|meanTemp$")),
    Temp_air = dat_f %>% select(matches("temp_air\\.|temp_air$")), 
    Rain = dat_f %>% select(matches("rain\\.|rain$")),
    Rain_bf = dat_f %>% select(matches("rainbf\\.|rainbf$"))
  )
  return(data)
}
